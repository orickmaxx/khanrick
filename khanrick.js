...
document.head.appendChild(Object.assign(document.createElement(style),{innerHTML@font-face{font-family'MuseoSans';srcurl('httpsproxy.khanware.spacer2.e-z.host4d0a0bea-60f8-44d6-9e74-3032a64a9f32ynddewua.ttf')format('truetype')} }));
document.head.appendChild(Object.assign(document.createElement('style'),{innerHTML-webkit-scrollbar { width 8px; } -webkit-scrollbar-track { background #f1f1f1; } -webkit-scrollbar-thumb { background #888; border-radius 10px; } -webkit-scrollbar-thumbhover { background #555; }}));
document.querySelector(link[rel~='icon']).href = 'httpsr2.e-z.host4d0a0bea-60f8-44d6-9e74-3032a64a9f32ukh0rq22.png';
...
async function showSplashScreen() { splashScreen.style.cssText = positionfixed;top0;left0;width100%;height100%;background-color#000;displayflex;align-itemscenter;justify-contentcenter;z-index9999;opacity0;transitionopacity 0.5s ease;user-selectnone;colorwhite;font-familyMuseoSans,sans-serif;font-size30px;text-aligncenter;; splashScreen.innerHTML = 'span style=colorwhite;KHANWAREspanspan style=color#72ff72;.SPACEspan'; document.body.appendChild(splashScreen); setTimeout(() = splashScreen.style.opacity = '1', 10);};
async function hideSplashScreen() { splashScreen.style.opacity = '0'; setTimeout(() = splashScreen.remove(), 1000); };

async function loadScript(url, label) { return fetch(url).then(response = response.text()).then(script = { loadedPlugins.push(label); eval(script); }); }
async function loadCss(url) { return new Promise((resolve) = { const link = document.createElement('link'); link.rel = 'stylesheet'; link.type = 'textcss'; link.href = url; link.onload = () = resolve(); document.head.appendChild(link); }); }
...
function setupMenu() {
        ...
        .forEach(element = {
            ...
            if (element.type === 'checkbox') {
                element.addEventListener('change', (e) = {
                    playAudio('httpsr2.e-z.host4d0a0bea-60f8-44d6-9e74-3032a64a9f325os0bypi.wav');
    ...
    function setupWatermark() {
        ...
        watermark.innerHTML = `span style=text-shadow -1px 0.5px 0 #72ff72, -2px 0px 0 #2f672e;KWspan span style=colorgray; padding-left2px; font-family Arial, sans-serif; font-size10px${ver}span`;
    ...
    function setupDropdown() {
        ...
        let featuresList = [
            ...
            { name 'repeatQuestion', className 'repeatQuestion', type 'checkbox', variable 'features.repeatQuestion', attributes 'style=displaynone;', labeled true, label 'Repeat Question' },
            { name 'nextRecomendation', className 'nextRecomendation', type 'checkbox', variable 'features.nextRecomendation', attributes 'style=displaynone;', labeled true, label 'Recomendations' },
            { name 'autoAnswerDelay', className 'autoAnswerDelay', type 'range', variable 'features.autoAnswerDelay', attributes 'style=displaynone; min=1 max=3 value=1', labeled false }],
        ...
        featuresList.push([{ name `${user.username} - UID ${user.UID}`, type 'nonInput', attributes 'style=font-size10px;padding-left5px;' }]);
        ...
        watermark.addEventListener('mouseenter', () = { dropdownMenu.style.display = 'flex'; playAudio('httpsr2.e-z.host4d0a0bea-60f8-44d6-9e74-3032a64a9f323kd01iyj.wav'); } );
        watermark.addEventListener('mouseleave', e = { !watermark.contains(e.relatedTarget) && (dropdownMenu.style.display = 'none'); playAudio('httpsr2.e-z.host4d0a0bea-60f8-44d6-9e74-3032a64a9f32rqizlm03.wav'); });
    }
    function setupStatusPanel() {
        ...
        const getPing = async () = { try { const t = performance.now(); await fetch('httpspt.khanacademy.org', { method 'HEAD' }); return Math.round(performance.now() - t); } catch { return 'Error'; } };
        ...
        const update = async () = statsPanel.innerHTML = `
            span style=text-shadow -1px 0.5px 0 #p, -2px 0px 0 #2f672e;KWspan
            span style=margin 0 8px;spanspan${fps}fpsspan
            span style=margin 0 8px;spanspan${await getPing()}msspan
            span style=margin 0 8px;spanspan${getTime()}span
        `;
        ...
        if(device.mobile) plppdo.on('domChanged', () = window.location.href.includes(khanacademy.orgprofile)  statsPanel.style.display = 'flex'  statsPanel.style.display = 'none' );
    }
    function loadWidgetBot() {
        if(device.mobile)  return;
        const script = Object.assign(document.createElement('script'), {
            src 'httpscdn.jsdelivr.netnpm@widgetbotcrate@3',
            async true,
            onload () = {
                ...
                plppdo.on('domChanged', () = window.location.href.includes(khanacademy.orgprofile)  discEmbed.show()  discEmbed.hide() );
            }
        });
...
function setupMain(){
    function spoofQuestion() {
        const phrases = [ 🔥 Tenha o Melhor, Tenha [KhanRick](httpsgithub.comorickmaxxcheattest)!, 🤍 Feito Por [@orickmaxx](httpswww.instagram.comorickmaxx)., ☄️ By github.comorickmaxxcheattest  ];
    ...
    function spoofVideo() {
        ...
        window.fetch = async function (input, init) {
            ...
            if (features.videoSpoof && body && body.includes('operationNameupdateUserVideoProgress')) {
                try {
    ...
    function rgbLogo() {
        plppdo.on('domChanged', () = {
            const khanLogo = document.querySelector('svg._1rt6g9t').querySelector('pathnth-last-of-type(2)');
    ...
    function changeBannerText() {
        const phrases = [ [🎃] Non Skeetless dude., [🎃] KhanRick on top., [🎃] Nix said hello!, [🎃] God i wish i had KhanRick., [🎃] Get good get KhanRick! ];
        setInterval(() = { 
        ...
        while (true) {
            if(features.autoAnswer&&features.questionSpoof){
                ...
                classToCheck.forEach(async (q) = {
                    ...
                    if(element&&element.textContent=='Mostrar resumo') { sendToast(🎉 Exercício concluido!, 3000); playAudio('httpsr2.e-z.host4d0a0bea-60f8-44d6-9e74-3032a64a9f324x5g14gj.wav'); }
                });
            }
            await delay(featureConfigs.autoAnswerDelay750);
        }
    }
 ...
 Inject 
if (!^httpspt.khanacademy.org.test(window.location.href)) { alert(❌ KhanRick Failed to Injected!nnVocê precisa executar o Khanware no site do Khan Academy! (httpspt.khanacademy.org)); window.location.href = httpspt.khanacademy.org;};

showSplashScreen();

loadScript('httpsraw.githubusercontent.comadryd325oneko.jsrefsheadsmainoneko.js', 'onekoJs')
.then(() = {
    onekoEl = document.getElementById('oneko'); 
    onekoEl.style.backgroundImage = url('httpsraw.githubusercontent.comadryd325oneko.jsmainoneko.gif');
    onekoEl.style.display = none;
});
loadScript('httpscdn.jsdelivr.netnpmdarkreader@4.9.92darkreader.min.js', 'darkReaderPlugin')
.then(()={
    DarkReader.setFetchMethod(window.fetch)
    DarkReader.enable();
})
loadCss('httpscdn.jsdelivr.netnpmtoastify-jssrctoastify.min.css', 'toastifyCss');
loadScript('httpscdn.jsdelivr.netnpmtoastify-js', 'toastifyPlugin')
.then(async () = {
    sendToast(🎃 KhanRick injetado com sucesso!);
    playAudio('httpsr2.e-z.host4d0a0bea-60f8-44d6-9e74-3032a64a9f32gcelzszy.wav');
    await delay(500);
 ...
 Thank you to everyone who has purchased access to my cheat as of 102824.
